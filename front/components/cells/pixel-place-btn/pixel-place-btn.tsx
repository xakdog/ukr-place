...
const PixelPlaceBtn: React.FC = () => {
  ...
  const isPending =
    pixelState.syncStatus === PixelSyncStatus.AWAITING_CONFIRMATION;
  const isTrashEnabled =
    !isPending && Object.keys(pixelState.syncing).length > 0;
  const isSendEnabled =
    !isPending && Object.keys(pixelState.syncing).length > 0;
  const isAddAvailable =
    !isPending && Object.keys(pixelState.syncing).length < 8;

  const paintPixel = useCallback(() => {
    setPixelState(
      pixelChangesActions.setStatus(PixelSyncStatus.COMMIT_CHANGES)
    );
  }, [setPixelState]);
  ...
  const savePixel = useRecoilCallback(
    ({ snapshot }) =>
      async () => {
        ...
        setPixelState(
          pixelChangesActions.paintPixel(color, pos.vector, randomId)
        );
      },
    [color, setPixelState]
  );
