...
export const useWalletProgram = (): WalletProgram | undefined => {
  ...
  const [ukrPlaceProgram, setUkrPlaceProgram] =
    // @ts-ignore
    useState<Program<typeof UkrPlace>>();
  const [canvasTileProgram, setCanvasTileProgram] =
    // @ts-ignore
    useState<Program<typeof CanvasTile>>();

  const inkWallet = useMemo(getWalletKey, []);
  const ukrPlaceId = useMemo(
    () => new PublicKey(UkrPlace.metadata.address),
    []
  );
  const canvasTileId = useMemo(
    () => new PublicKey(CanvasTile.metadata.address),
    []
  );
...
function getWalletKey() {
  ...
  if (key) {
    try {
      ...
      console.error(e);
      console.error(new Error("Secret key was regenerated"));
    }
  }
