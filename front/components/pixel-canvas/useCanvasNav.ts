...
export const useCanvasNav = ({ container, onClick }: canvasNavArgs) => {
  ...
  const updateGlobalPos = useCallback(
    (pos: Vector) => {
      const intPos = new Vector(Math.floor(pos.x), Math.floor(pos.y));

      setGlobalPos((state) =>
        isSameVectors(state.vector, intPos)
          ? state
          : {
              vector: intPos,
              outOfBounds:
                intPos.x < 0 ||
                intPos.x > IMAGE_WIDTH ||
                intPos.y < 0 ||
                intPos.y > IMAGE_HEIGHT,
            }
      );
    },
    [setGlobalPos]
  ...
  useGesture(
    {
      ...
      onDragEnd: ({ offset: [ox, oy], target }) => {
        ...
        if (target !== container.current?.parentNode) return;
    ...
    {
      target: container.current?.parentElement || documentBody,
    }
  );
