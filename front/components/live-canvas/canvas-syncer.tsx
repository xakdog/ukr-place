...
export const CanvasSyncer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    connection
      ...
      .then(res => res.map(rec => canvasTile.coder.accounts.decode('CanvasTile', rec.account.data as Buffer)))
      .then(res => Promise.all(res.map(c =>
        pixelChangesTransform.colorsArrayToImage(c.pixels, new Vector(c.position.x, c.position.y))
      )))
      .then(res => {
    ...
    connection.onProgramAccountChange(programId, change => {
      const acc = canvasTile.coder.accounts.decode('CanvasTile', change.accountInfo.data as Buffer);
      const tile = pixelChangesTransform.colorsArrayToImage(acc.pixels, new Vector(acc.position.x, acc.position.y));
